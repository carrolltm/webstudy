...
https://blog.csdn.net/weixin_45325238/article/details/104968195
...
bind函数**多次调用**会已**第一次绑定**的this为准，softbind已**最后一次绑定**传入的this为准；
...
例子：对于一个滚动事件：下面你会发现不断的打印"onscroll"，在短短的几百毫秒内触发了近百次，执行频率太高了
...
```js
    <script>
      ...
      document.body.onscroll = debounce(handle, 1000);
    </script>
```
...
[三种解析](https://www.cnblogs.com/leejersey/p/5191924.html)

**方法一：**typeof

**方法二**：instanceof

**方法三（最重要的一点）**：Object.prototype.toString.call()
...
　  (a.constructor == Array)  // a实例所对应的构造函数是否为Array? true or false      // 参考js高程的构造函数模式。
